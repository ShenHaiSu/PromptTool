/**
 * 领域模型 — 对标 src/engine/models.py
 * dataclass → interface / class，bool 0/1 → boolean，hash 保持 md5 一致
 */
...
// ------------------------------------------------------------------
// SelectedItem / IRSegment / PromptIR / AssemblyConfig
// ------------------------------------------------------------------
...
function md5Hex(str: string): string {
  ...
  function rstr2binl(input: string): number[] {
    const output: number[] = Array(((input.length + 64) >> 9 << 4) + 16).fill(0)
    for (let i = 0; i < input.length; i++) output[i >> 2] |= input.charCodeAt(i) << (i % 4 << 3)
    return output
  }
  ...
  function str2rstrUTF8(input: string): string { return unescape(encodeURIComponent(input)) }
...
export class PromptIR {
  ...
  hash(): string {
    const parts: [string, string, string][] = this.segments.map((s) => [
      s.dimensionKey,
      s.text,
      (Math.round(s.weight * 10) / 10).toFixed(1),
    ])
    const raw = parts.map(([d, t, w]) => `${d}:${t}:${w}`).join('|')
    return md5Hex(raw)
  }
...
export function defaultAssemblyConfig(): AssemblyConfig {
  return {
    separator: ', ',
